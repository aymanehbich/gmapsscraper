...
def clean_and_deduplicate_leads(input_file, output_file, default_city=None, default_country=None, default_category=None):
    ...
    with open(input_file, "r", encoding="utf-8") as f:
        for line in f:
            ...
            unique_id = item.get("placeId") or item.get("cid") or item.get("title")
            ...
            if isinstance(raw_emails, list):
                for e in raw_emails:
                    if isinstance(e, str) and "@" in e:
                        ...
                        emails_list.append(e["email"].strip())
            elif isinstance(raw_emails, str) and "@" in raw_emails:
                emails_list.append(raw_emails.strip())
            ...
            website = item.get("web_site") or item.get("website") or item.get("url") or item.get("site") or item.get("link") or ""
    ...
    print(
        f"Successfully processed {len(cleaned_leads)} unique leads. Saved to {output_file}"
    )
